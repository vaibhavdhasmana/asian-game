...
async function getSettingsDoc() {
  ...
  if (!doc) {
    doc = await AdminSettings.create({
      currentDay: "day1",
      currentSlot: 1,
      slotsPerDay: { day1: 1, day2: 1, day3: 1, day4: 1 },
      groups: {
        day2: [
    ...
    // Ensure day4 is present in existing docs
    if (!doc.slotsPerDay.day4) doc.slotsPerDay.day4 = 1;
    if (!doc.groups.day4 || doc.groups.day4.length === 0) {
      doc.groups.day4 = [
        { key: "queen", label: "The Queen's Palette", logo: "" },
        { key: "king", label: "The King's Tone", logo: "" },
        { key: "bishop", label: "The Bishop's Gambit", logo: "" },
...
router.post("/settings/day", requireAdmin, async (req, res) => {
  ...
  if (!["day1", "day2", "day3", "day4"].includes(String(currentDay))) {
...
router.post("/settings/slots-per-day", requireAdmin, async (req, res) => {
  ...
  const s = await getSettingsDoc();
  s.slotsPerDay = s.slotsPerDay || { day1: 1, day2: 1, day3: 1, day4: 1 };
...
router.get("/content/list", requireAdmin, async (req, res) => {
  try {
    ...
    const q = {};
    if (day && ["day1", "day2", "day3", "day4"].includes(String(day)))
      q.day = String(day);
    if (
      game &&
      ["quiz", "crossword", "wordSearch", "jigsaw"].includes(String(game))
    )
      q.game = String(game);
...
router.post("/settings/groups", requireAdmin, async (req, res) => {
  ...
  if (!["day2", "day3", "day4"].includes(dayKey))
    return res.status(400).json({ message: "Invalid day" });
  // Support legacy payload `colors: ["red", ...]` by mapping to label-less groups
  if (Array.isArray(colors)) {
    s.groups[dayKey] = colors.map((c) => ({
      key: String(c),
      label: String(c),
      logo: "",
    }));
  } else if (Array.isArray(groups)) {
    s.groups[dayKey] = groups.map((g) => ({
      key: g.key,
      label: g.label || g.key,
      logo: g.logo || "",
    }));
...
router.post("/users/admin", async (req, res) => {
  ...
  if (!uuid || typeof isAdmin !== "boolean")
    return res.status(400).json({ message: "uuid and isAdmin required" });
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const file = req.file;
      if (!day || !game || !file)
        return res.status(400).json({ message: "day, game and file required" });
      const dayKey = String(day);
      ...
      if (file.mimetype && file.mimetype.startsWith("image/")) {
        ...
        const fname = `${dayKey}_${gameKey}${
          slotNum != null ? "_slot" + slotNum : ""
        }${groupKey ? "_" + groupKey : ""}_${version}${ext}`;
        ...
        try {
          ...
          if (gameKey === "quiz") {
            ...
            for (let i = 0; i < body.length; i++) {
              ...
              const optA =
                get("opta") || get("opt1") || get("optiona") || r[2] || "";
              const optB =
                get("optb") || get("opt2") || get("optionb") || r[3] || "";
              const optC =
                get("optc") || get("opt3") || get("optionc") || r[4] || "";
              const optD =
                get("optd") || get("opt4") || get("optiond") || r[5] || "";
              let ci = get("correctindex");
              if (ci == null) {
                const cad = (get("correct(a-d)") || "")
                  .toString()
                  .trim()
                  .toUpperCase();
                if (cad === "A") ci = 0;
            ...
            if (idxWord >= 0) {
              words = body
                .map((r) => (r[idxWord] || "").toString().toUpperCase())
                .filter(Boolean);
            } else {
              words = body
                .map((r) => (r[0] || "").toString().toUpperCase())
                .filter(Boolean);
            }
...
router.post("/content/json", requireAdmin, async (req, res) => {
  try {
    ...
    if (!usePayload || typeof usePayload !== "object") {
      return res
        .status(400)
        .json({ message: "payload required (or questions[] for quiz)" });
    }
...
// Body: { uuid: string, day?: 'day1'|'day2'|'day3'|'day4', slot?: number, game?: 'quiz'|'crossword'|'wordSearch'|'jigsaw' }
